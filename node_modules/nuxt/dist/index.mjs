...
import { useNuxt, resolveFiles, logger, defineNuxtModule, addPlugin, addTemplate, addComponent, updateTemplates, useNitro, addVitePlugin, addWebpackPlugin, addBuildPlugin, addTypeTemplate, findPath, addImportsSources, tryResolveModule, isIgnored, tryUseNuxt, resolvePath, resolveAlias, addPluginTemplate, normalizeModuleTranspilePath, resolveNuxtModule, resolveIgnorePatterns, createResolver, nuxtCtx, installModule, addRouteMiddleware, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
...
async function generateRoutesFromFiles(files, options = {}) {
  const routes = [];
...
async function getRouteMeta(contents, absolutePath) {
  ...
  const extraneousMetaKeys = pageMetaArgument.properties.filter((property) => property.type === "Property" && property.key.type === "Identifier" && !extractionKeys.includes(property.key.name)).map((property) => property.key.name);
...
function getPropsToString(bindings, vfor) {
...
const nuxtImportProtections = (nuxt, options = {}) => {
  const patterns = [];
  patterns.push([
