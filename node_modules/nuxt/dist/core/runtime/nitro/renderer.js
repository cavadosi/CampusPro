...
const sharedPrerenderCache = import.meta.prerender && process.env.NUXT_SHARED_DATA ? {
  get(key) {
    ...
    return useStorage("internal:nuxt:prerender:shared").getItem(key);
  },
  async set(key, value) {
    sharedPrerenderPromises.set(key, value);
    return useStorage("internal:nuxt:prerender:shared").setItem(key, await value).finally(() => sharedPrerenderPromises.delete(key));
  }
} : null;
...
export default defineRenderHandler(async (event) => {
  ...
  const isRenderingIsland = process.env.NUXT_COMPONENT_ISLANDS && event.path.startsWith("/__nuxt_island");
