...
import { isFileServingAllowed, isCSSRequest } from 'vite';
...
import { parseURL, parseQuery, joinURL, withoutLeadingSlash, withTrailingSlash, withoutBase } from 'ufo';
...
async function writeManifest(ctx, css = []) {
  ...
  const clientManifest = ctx.nuxt.options.dev ? devClientManifest : await fse.readJSON(manifestFile);
  ...
  await fse.writeFile(resolve$1(serverDist, "client.manifest.json"), stringifiedManifest, "utf8");
  await fse.writeFile(resolve$1(serverDist, "client.manifest.mjs"), "export default " + stringifiedManifest, "utf8");
  if (!ctx.nuxt.options.dev) {
  ...
  return {
    ...
    renderChunk(_code, chunk) {
      ...
      for (const moduleId of [chunk.facadeModuleId, ...chunk.moduleIds].filter(Boolean)) {
        if (options.mode === "client") {
          (_c = options.clientCSSMap)[moduleId] || (_c[moduleId] = /* @__PURE__ */ new Set());
          if (isCSS(moduleId)) {
            if (isVue(moduleId)) {
