		...
		<div class="flow-root pb-16 pt-24 sm:pt-32 lg:pb-0">
			<div class="mx-auto max-w-7xl px-6 lg:px-8">
				<div class="relative z-10">
					...
					<p class="mx-auto mt-4 max-w-2xl text-center text-lg leading-8 text-black/60">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Velit numquam eligendi quos odit doloribus molestiae voluptatum quos odit doloribus.</p>
					<div class="mt-16 flex justify-center">
						<RadioGroup v-model="frequency" class="grid grid-cols-2 gap-x-1 rounded-full bg-white p-1 text-center text-xs font-semibold leading-5 text-black">
				...
				<div class="relative mx-auto mt-10 grid max-w-md grid-cols-1 gap-y-8 lg:mx-0 lg:-mb-14 lg:max-w-none lg:grid-cols-3">
					...
					<div v-for="tier in tiers" :key="tier.id" :class="[tier.featured ? 'z-10 bg-white shadow-xl ring-1 ring-gray-900/10' : 'bg-primary/50 ring-1 ring-white/10 lg:bg-transparent ', 'relative rounded-2xl']">
						<div class="p-8 lg:pt-12 xl:p-10 xl:pt-14">
							...
							<div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:flex-col lg:items-stretch">
								<div class="mt-2 flex items-center gap-x-4">
									...
									<div class="text-sm leading-5">
										...
										<p :class="tier.featured ? 'text-gray-500' : 'text-gray-300'">{{ `Billed ${frequency.value}` }}</p>
									</div>
								</div>
								<a
									:href="tier.href"
									...
									:class="[
										tier.featured ? 'bg-primary shadow-sm hover:bg-primary-300 focus-visible:outline-primary' : 'bg-white/10 hover:bg-white/20 focus-visible:outline-white',
							...
							<div class="mt-8 flow-root sm:mt-10">
								<ul role="list" :class="[tier.featured ? 'divide-gray-900/5 border-gray-900/5 text-gray-600' : 'divide-white/20 border-white/5 text-white', '-my-2 divide-y border-t text-sm leading-6 lg:border-t-0']">
					...
					<div class="mx-auto max-w-2xl space-y-16">
						<div v-for="tier in tiers" :key="tier.id" class="border-t border-gray-900/10">
							<div :class="[tier.featured ? 'border-primary' : 'border-transparent', '-mt-px w-72 border-t-2 pt-10 md:w-80']">
								<h3 :class="[tier.featured ? 'text-primary' : 'text-gray-900', 'text-sm font-semibold leading-6']">{{ tier.name }}</h3>
							...
							<div class="mt-10 space-y-10">
								<div v-for="section in sections" :key="section.name">
									...
									<div class="relative mt-6">
										...
										<div :class="[tier.featured ? 'ring-2 ring-primary' : 'ring-1 ring-gray-900/10', 'relative rounded-lg bg-white shadow-sm sm:rounded-none sm:bg-transparent sm:shadow-none sm:ring-0']">
												...
												<div v-for="feature in section.features" :key="feature.name" class="flex items-center justify-between px-4 py-3 sm:grid sm:grid-cols-2 sm:px-0">
													...
													<dd class="flex items-center justify-end sm:justify-center sm:px-4">
														...
														<template v-else>
															...
															<IconHeroicons:x-mark-20-solid v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
										...
										<!-- Fake card border -->
										<div aria-hidden="true" :class="[tier.featured ? 'ring-2 ring-primary' : 'ring-1 ring-gray-900/10', 'pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 rounded-lg sm:block']" />
									</div>
								</div>
							</div>
						</div>
					...
					<div class="grid grid-cols-4 gap-x-8 border-t border-gray-900/10 before:block">
						<div v-for="tier in tiers" :key="tier.id" aria-hidden="true" class="-mt-px">
							<div :class="[tier.featured ? 'border-primary' : 'border-transparent', 'border-t-2 pt-10']">
								<p :class="[tier.featured ? 'text-primary' : 'text-gray-900', 'text-sm font-semibold leading-6']">{{ tier.name }}</p>
								<p class="mt-1 text-sm leading-6 text-gray-600">{{ tier.description }}</p>
							</div>
						</div>
					</div>
						...
						<div v-for="section in sections" :key="section.name">
							...
							<div class="relative -mx-8 mt-10">
										...
										<tr v-for="(feature, featureIdx) in section.features" :key="feature.name">
											...
											<td v-for="tier in tiers" :key="tier.id" class="relative w-1/4 px-4 py-0 text-center">
												<span class="relative h-full w-full py-3">
													<span v-if="typeof feature.tiers[tier.name] === 'string'" :class="[tier.featured ? 'font-semibold text-primary' : 'text-gray-900', 'text-sm leading-6']">{{ feature.tiers[tier.name] }}</span>
													<template v-else>
														<IconHeroicons:check-20-solid v-if="feature.tiers[tier.name] === true" class="mx-auto h-5 w-5 text-accent" aria-hidden="true" />
														<IconHeroicons:x-mark-20-solid v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
														<span class="sr-only">{{ feature.tiers[tier.name] === true ? 'Yes' : 'No' }}</span>
								...
								<div class="pointer-events-none absolute inset-x-8 inset-y-0 grid grid-cols-4 gap-x-8 before:block" aria-hidden="true">
									<div v-for="tier in tiers" :key="tier.id" :class="[tier.featured ? 'ring-2 ring-primary' : 'ring-1 ring-gray-900/10', 'rounded-lg']" />
								</div>
							</div>
						</div>
					</div>
